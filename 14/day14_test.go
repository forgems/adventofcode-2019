...
5 BHXH, 4 VRPVC => 5 LTCX`, 2210736},
	...
	for i := range testData {
		...
		t.Run(fmt.Sprintf("Test %d", i), func(t *testing.T) {
			recipe := ReadInput(strings.NewReader(testData[i].recipe))
			refinery := Refinery{}
			ore := refinery.Produce("FUEL", 1, recipe)
			if ore != testData[i].ore {
				t.Errorf("Invalid amount of ore %d (should be %d)", ore, testData[i].ore)
			}
		})
	}
